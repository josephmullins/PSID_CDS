...
C <- read.csv("~/Dropbox/PSID_CDS/PSID_CLEAN/ChildPanelCDS.csv") %>% mutate(X=NULL) %>%
  filter(year==2002) %>%
  select(KID,state,chcare,m_wage,tau_m)


price_ext <- read.csv("~/Dropbox/JPE_Child_Devp_2020/Prices/childcare_prices2002.csv") %>% mutate(state = stateid,pc_infant = p_icent_e_cps_cpkt,pf_infant = p_ifcc_e_cps_cpkt,pc_four = p_yocent_e_cps_cpkt,pf_four = p_yofcc_e_cps_cpkt)

price_good <- read.csv("~/Dropbox/JPE_Child_Devp_2020/Prices/RelativePricesGoods.csv") %>% mutate(state = GeoFips, price_good = X2002) %>% select(state,price_good)
price_serv <- read.csv("~/Dropbox/JPE_Child_Devp_2020/Prices/RelativePricesGoods.csv") %>% mutate(state = GeoFips, price_serv = X2002) %>% select(state,price_serv)


time_inv <- read.csv("../../../data-cds/time-diary/ActiveTimePanel.csv") %>%
  filter(year==2002) %>%
...
D <- C %>%
  ...
  merge(time_inv) %>%
  mutate(chcare2 = formal*pc_four/52 + informal*pf_four/52,chcare3 = chcare2+pc_four*relative/52) %>%
