...
C = read.csv("../../data-main/childbirth/Childbirth.csv") %>% 
  ...
  select(KID) %>%
  unique() %>%
  inner_join(KID) %>% #<- keep only women whose children are in the CDS (THIS ALSO DROPS NON-CDS CHILDREN. DOESN'T WORK!!!!)
  group_by(MID) %>%
...
m_pc97 <- read.csv("~/Dropbox/PSID_CDS/PSID_CLEAN/AssessmentMother.csv") %>% mutate(X=NULL)
