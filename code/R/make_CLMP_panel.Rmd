...
prices <- read.csv("../../../JPE_Child_Devp_2020/PSID/CleanData/prices_lance.csv") %>%
...
d <- child_panel %>%
  ...
  select(KID,year,LW02,tau_m,tau_f,chcare,formal,informal,relative,goods,Race,m_ed,f_ed)

d <- child_panel %>%
  filter(year == 1997) %>%
  mutate(LW97 = (LW_std - 100)/15, AP97 = (AP_std - 100)/15) %>%
  select(KID, LW97, AP97) %>%
  inner_join(d)

write.csv(child_panel, "data-derived/gmm_data.csv")
