...
T2 <- read.csv("~/Dropbox/PSID_CDS/PSID_RAW/TD07full.csv")%>%
  ...
  mutate(COLG_B = if_else(COLG_B==5, as.integer(0), as.integer(COLG_B)),COLG_C = if_else(COLG_C==5, as.integer(0), as.integer(COLG_C)),
         COLH_B = if_else(COLH_B==5, as.integer(0), as.integer(COLH_B)), COLH_C = if_else(COLH_C==5, as.integer(0), as.integer(COLH_C)),#<- 0 corresponds to activity being personal, private, school, or work according to the codebook - 5- activity not selected - make it 0 to be consistent with 1997, 2002
