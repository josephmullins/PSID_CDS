...
TD_agg <- TD %>% 
  group_by(KID,year) %>% 
  #summarise(tau_m = sum(DURATION*wght*COLG_B,na.rm = TRUE)/3600, tau_f = sum(DURATION*wght*COLG_C,na.rm=TRUE)/3600, tau_both = sum(DURATION*wght*(COLG_B + (1-COLG_B)*COLG_C),na.rm=TRUE)/3600, tau_m2 = sum(time2ind*DURATION*wght*(COLG_B + (1-COLG_B)*COLH_B),na.rm = TRUE)/3600, tau_f2 = sum(time2ind*DURATION*wght*(COLG_C + (1-COLG_C)*COLH_C),na.rm=TRUE)/3600)
...
```{r}
E = read.csv("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/Exp2002.csv") 
E[E>=99998] <- NA
E <- E %>%
...
```{r}
E2 <- read.csv("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/Exp2002v2.csv")
E2[E2==9998] <- NA
E2[E2==9999] <- NA
E2[E2==99998] <- NA
E2[E2==99999] <- NA
...
write.csv(E[,cols],"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/Expenditures.csv")
...
```{r}
Ch <- read.csv("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/Childcare.csv") %>% #<- read in the data
  mutate(KID = ER30001*1000 + ER30002) %>%
  ...
  mutate(Arrange97 = case_when(Q1H14==1 | Q1H14==2 | Q1H14==3 ~ "Relative",
...
Ch <- Ch %>%
  ...
  mutate(Arrange02 = case_when(Q21C12==1 | Q21C12==2 | Q21C12==3 ~ "Relative",
...
ch_hist <- readxl::read_excel("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/Childcare_history.xlsx") %>%
  mutate(KID = ER30001*1000 + ER30002)
...
ch_hist %>% select(KID,CaretypeBroad,Arrange97,Exp97Tot,Hours97Tot,Arrange,Hours97,Exp97) %>%
  ...
  write.csv("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/ChildcarePreK97.csv")
...
write.csv(Cpanel,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/ChildCarePanel.csv")
...
write.csv(d,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/Main_childcare.csv")
```
...
```{r}
PPE <- readxl::read_excel("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/CDS_PPE.xlsx") %>%
  mutate(KID = ER30001*1000 + ER30002) %>%
  rename(PPE = Q12A27) %>%
...
write.csv(PPE,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/PPE.csv")
```
...
```{r}
D <- readxl::read_excel("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/ChildAssessments.xlsx") %>% 
  mutate(KID = ER30001*1000 + ER30002)
...
write.csv(A,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/AssessmentPanel.csv")
...
We start by loading the childbirth history file, which we will use to identify mothers and arrabge their fertility history into a panel dataset. It will be useful to know the following mapping between variable names and variables:
...
```{r}
cds <- read.csv("~/Dropbox/Data/PSID_CDS/MotherIndex.csv") %>% select(c("MID")) %>% unique() #<- this file contains all the women whose children end up in the CDS

C = read.csv("~/Dropbox/Data/PSID_MAIN/Childbirth.csv") %>% mutate(X = NULL) %>% #<- load childbirth file
...
D <- C %>% group_by(MID) %>% do(MakePanel(.)) %>% filter(y_first>=1960) %>%
  mutate(year = year + 1) #<- convert the calendar year to interview year for the rest of the merge
...
```{r}
Ind = read.csv("~/Dropbox/Data/PSID_MAIN/ind2011er/IndFileVertical.csv") %>% 
  mutate(X = NULL) %>% 
  mutate(MID = intnum68*1000 + pernum,mar_stat = mpair>0)

```

Since we would like to see marital status in some non-interview years, we use the following year's marital status for 1999, 2001, and 2004. 
:
```{r}
...
Next we load in a panel of state codes, which we merge with the cross-year individual file. We then merge that file the a file of State Codes so that we can link the PSID's state code with FIPS and SOI. We also merge, based on the household interview number and year, annual household expenditures on child care (cleaned in a previous section).
...
ch_care <- read.csv("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/Main_childcare.csv") %>% mutate(X=NULL)

D <- read.csv("~/Dropbox/Data/PSID_MAIN/StateVertical.csv") %>% #<- state of residence panel data
  mutate(X=NULL) %>%
  inner_join(Ind) %>% #<- merge with Individual file
...
2. Partners are identified by the fact that they are not successfully matched in this merge, meaning that *age_mother* will be missing. We keep those observations and rename the variables in order to merge them once more with the the panel of mothers.
...
We also use this file to merge with FID for each spouse to deduce the spouse's age.
...
```{r}
R <- read.csv("~/Dropbox/Data/PSID_MAIN/RaceHorizontal.csv") %>% mutate(X = NULL) %>% 
  rename(MID = ID)

educ = read.csv("~/Dropbox/Data/PSID_MAIN/HGC.csv") %>% 
...
cpi = read.csv("~/Dropbox/Data/CPI-U.csv") %>% 
  rename(year = YEAR) %>%
  mutate(CPIU = CPIU/CPIU[53]) %>% #<- makes year 2000 the base year
  mutate(year = year+1) #<- convert calendar to interview year
...
```{r}
L = read.csv("~/Dropbox/Data/PSID_MAIN/LaborFile.csv") %>% mutate(X = NULL) #<- contains the hours and earnings of the "head" and the "wife" which can be matched to individuals via sequence number
...
L <- D %>%
  ...
  select(MID,FID,year,m_earn,m_hrs,m_wage,f_earn,f_hrs,f_wage) %>%
  mutate(year = year - 1) #<- convert to calendar instead of interview year
```


Since the PSID went to a biennial format after 1997, the file above does not have data for 1998,2000, and so on. A supplementary set of questions is asked about the family members from the previous year. These are sometimes called the "T2" supplement, which the chunk of code below cleans into a panel dataset for those missing years. These data were collected from the "Individual Data Index" from the PSID's data center tool, so they are linked to individuals already. The code below renames the variables and reshapes them into a panel. The reporting of earnings in these years is slightly different from the main questions, so we keep only observations that are reported in annual terms (the large majority).

```{r}
T2 <- readxl::read_xlsx("~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_RAW/T2_alt.xlsx") %>%
...
T2m <- D %>%
  ...
  select(-FID) #<- these are mothers so drop the FID variable
...
T2f <- D %>%
  ...
  select(-MID) #<- these are fathers so drop the MID variable
...
D <- D %>%
  select(MID,FID,year) %>%
  mutate(year = year-1) %>% #<- convert interview year to calendar year 
  inner_join(T2m) %>%
  left_join(T2f) %>%
  ...
  mutate(year = year + 1) %>% # <- convert back to interview year for the merge with D
  right_join(D) %>%
  mutate(year = year - 1) #<- final conversion back to calendar year

```

We save the panel dataset.
```{r}
write.csv(D,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/MotherPanelCDS.csv")
```
...
cds <- read.csv("~/Dropbox/Data/PSID_CDS/MotherIndex.csv") %>% mutate(X=NULL)
...
write.csv(D_main,"~/Dropbox/JPE_Child_Devp_2020/PSID/PSID_CLEAN/ChildPanelCDS.csv")
```
